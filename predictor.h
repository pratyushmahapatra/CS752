...
// Global values needed for table selection
int HitBank_taken;
int AltBank_taken;
int BI_taken;
int Loop_pred_taken;
int gtable_update_counter;
int gtable_update_index[10]; //if more than 10 updates happen per run, it'll seg fault
int btable_update_counter;
int btable_update_index;
...
public:
  ...
  void store_restore_ghist (int new_proc, int old_proc)
  {
    if (procs[old_proc] == 0) {
	//do nothing
	procs[old_proc] = 1;
...
// gindex computes a full hash of PC, ghist and phist
  ...
  void baseupdate (bool Taken)
  {
    ...
    btable_update_counter = 1;
    btable_update_index = BI;
  };
...
//USE_ALT_ON_NA is positive  use the alternate prediction
	...
	if ((!Huse_alt_on_na)
	  ...
	  if (AltBank > 0) {
	    AltBank_taken = 1;
	    //printf("AltBank Prediction Taken. Index : %d\n", AltBank);
	  }
          else { 
	    BI_taken = 1;
	    //printf("BI Prediction Taken. Index : %d\n", BI);
          }
	}
...
#ifndef SC
    printf("%d |%d |%d |%d |%d |", BI_taken, BI, HitBank_taken | AltBank_taken, ((HitBank_taken) ? HitBank : (AltBank_taken ? AltBank : 0) ), ((HitBank_taken) ? GI[HitBank] : (AltBank_taken ? GI[AltBank] : 0) ));
    return (tage_pred);
#endif
...
// PREDICTOR UPDATE
			...
			bool predDir, UINT64 branchTarget)
  {
    ...
    gtable_update_counter = 0;
    for (int i = 0; i < 10; i++)
      gtable_update_index[i] = 0;
    btable_update_counter = 0;
    btable_update_index = 0;
...
//even if the overall prediction was false
		...
		//	printf ("Update : Alt_on_na table incremented. Index %d\n", INDUSEALT);
	        //} else {
		//	printf ("Update : Alt_on_na table decremented. Index %d\n", INDUSEALT);
	        //}
	      }
...
// the replacement is optimized with a single u bit: 0.2 %
			...
			//printf ("Update : gtable.ctr %d GI[%d]\n", gtable[i][GI[i]].ctr, i);
			NA++;
			if (T <= 0)
			  {
			    break;
			...
			//printf ("Update : gtable.ctr %d GI[%d]\n", gtable[i][GI[i]].ctr, i);
		      ...
		      gtable_update_counter++;
		      gtable_update_index[gtable_update_counter - 1] = i;
...
//update predictions
	  ...
	  if (LongestMatchPred != resolveDir)
	      ...
	      if (AltBank > 0)
		{
		  ctrupdate (gtable[AltBank][GI[AltBank]].ctr,
			     resolveDir, CWIDTH);
		  //printf ("Update : gtable[AltBank].ctr %d\n", gtable[AltBank][GI[AltBank]].ctr);
		  gtable_update_counter++;
		  gtable_update_index[gtable_update_counter - 1] = AltBank;
		}
	      if (AltBank == 0)
	...
	gtable_update_counter++;
	gtable_update_index[gtable_update_counter - 1] = HitBank;
//sign changes: no way it can have been useful
      ...
      if (LongestMatchPred == resolveDir)
	{
	  if (gtable[HitBank][GI[HitBank]].u < (1 << UWIDTH) - 1) {
	    ...
	    //printf ("Update : gtable[HitBank].u %d\n", gtable[HitBank][GI[HitBank]].u );
	  }
	  gtable_update_counter++;
	  gtable_update_index[gtable_update_counter - 1] = HitBank;
	}
//END TAGE UPDATE
    ...
    printf("%d |%d |%d |", btable_update_counter, btable_update_index, btable[BI].pred);
    printf("%d |", (gtable_update_counter > 0));
    for (int a = 0; a < gtable_update_counter; a++)
      printf("%d,", gtable_update_index[a]);
    printf("| ");
    for (int a = 0; a < gtable_update_counter; a++)
      printf("%d,", GI[gtable_update_index[a]]);
    printf("| ");
    for (int a = 0; a < gtable_update_counter; a++)
      printf("%d,", gtable[gtable_update_index[a]][GI[gtable_update_index[a]]].ctr);
    printf("| ");
    for (int a = 0; a < gtable_update_counter; a++)
      printf("%d,", gtable[gtable_update_index[a]][GI[gtable_update_index[a]]].u );
