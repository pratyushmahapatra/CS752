...
//ver2    myBtb.clear();
  ...
  ///////////////////////////////////////////////
      ...
      UINT64 branchTarget;
      UINT64 numIter = 0;
	uint64_t branhcPhyAddr;
      UINT64 uniqueBranches=0;
	 std::map<uint64_t, int> unique_br;
...
//          }
	  ...
	  branhcPhyAddr = it->getSrcNode()->brPhyAddr();
//Checking for unique branches
          if(unique_br.find(branhcPhyAddr) != unique_br.end())
                unique_br[branhcPhyAddr]++;
          else
                unique_br.insert(std::make_pair(branhcPhyAddr, 1));
...
/************************************************************************************************************/
	...
	std::map<uint64_t, int>::iterator it_2 = unique_br.begin();
    while(it_2 != unique_br.end())
    {
        uniqueBranches++;
        //myfile <<it_2->first<<" :: "<<it_2->second<<std::endl;
        it_2++;
    }
    ...
    //NOTE: competitors are judged solely on MISPRED_PER_1K_INST. The additional stats are just for tuning your predictors.
      printf("  TRACE \t : NUM_INSTRUCTIONS \t : NUM_BR                      \t : NUM_UNCOND_BR               \t : NUM_CONDITIONAL_BR          \t : NUM_MISPREDICTIONS          \t : MISPRED_PER_1K_INST         \t : NUM_UNIQUE_BR \n");
      printf("  %s : %10llu : %10llu  : %10llu :  %10llu :  %10llu : %10.4f : %10llu \n", trace_path.c_str(), total_instruction_counter, branch_instruction_counter-1, uncond_branch_instruction_counter, cond_branch_instruction_counter, numMispred, 1000.0*(double)(numMispred)/(double)(total_instruction_counter), uniqueBranches); 
}
