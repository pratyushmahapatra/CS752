...
uint8_t ghist_proc[100][HISTBUFFERLENGTH];
int ptghist;
int ptghist_proc[100];
long long phist;		//path history
long long phist_proc[100];		//path history
...
bentry *btable_proc[100];			//bimodal TAGE table
gentry *gtable[NHIST + 1];	// tagged TAGE tables
gentry *gtable_proc[100][NHIST + 1];	// tagged TAGE tables
int m[NHIST + 1];
int TB[NHIST + 1];
int logg[NHIST + 1];
...
lentry *ltable_proc[100];			//loop predictor table
...
public:
  ...
  void switch_contexts(int new_proc, int old_proc)
  {
    ...
    for (int i = 0; i < HISTBUFFERLENGTH; i++) {
      ghist_proc[old_proc][i] = ghist[i];
      if (procs[new_proc]) { 
        ghist[i] = ghist_proc[new_proc][i];
      }
    }
    phist_proc[old_proc] = phist;
    
    if (phist_proc[old_proc] && phist_proc[new_proc])
      printf("phist_proc[old_proc] = %lld , phist_proc[new_proc] = %lld \n", phist_proc[old_proc], phist_proc[new_proc]);

    if (procs[new_proc]) {
      phist = phist_proc[new_proc];
    }
        
    for (int i = 0; i < (1 << LOGB); i++) {
      btable_proc[old_proc][i].pred = btable[i].pred;
      btable_proc[old_proc][i].hyst = btable[i].hyst;
      if (procs[new_proc]) {
        btable[i].pred = btable_proc[new_proc][i].pred;
        btable[i].hyst = btable_proc[new_proc][i].hyst;
      }
    }
        		
    for (int i = 0; i < (NBBANK[0] * (1 << LOGG)); i++) {
      gtable_proc[old_proc][1][i].ctr = gtable[1][i].ctr;
      gtable_proc[old_proc][1][i].u   = gtable[1][i].u  ;
      gtable_proc[old_proc][1][i].tag = gtable[1][i].tag;
      if (procs[new_proc]) {
        gtable[1][i].ctr = gtable_proc[new_proc][1][i].ctr;
        gtable[1][i].u   = gtable_proc[new_proc][1][i].u  ;
        gtable[1][i].tag = gtable_proc[new_proc][1][i].tag;
      }
    }

    for (int l = 2; l <= NHIST; l++) {
      for (int i = 0; i < (NBBANK[1] * (1 << LOGG)); i++) {
        gtable_proc[old_proc][l][i].ctr = gtable[l][i].ctr;
        gtable_proc[old_proc][l][i].u   = gtable[l][i].u  ;
        gtable_proc[old_proc][l][i].tag = gtable[l][i].tag;
        if (procs[new_proc]) {
          gtable[l][i].ctr = gtable_proc[new_proc][l][i].ctr;
          gtable[l][i].u   = gtable_proc[new_proc][l][i].u  ;
          gtable[l][i].tag = gtable_proc[new_proc][l][i].tag;
        }
      }
    }
    ...
    //  ltable_proc[old_proc][i].confid      = ltable[i].confid     ;
    //  ltable_proc[old_proc][i].CurrentIter = ltable[i].CurrentIter;
    //  ltable_proc[old_proc][i].NbIter      = ltable[i].NbIter     ;
    //  ltable_proc[old_proc][i].TAG         = ltable[i].TAG        ;
    //  ltable_proc[old_proc][i].age         = ltable[i].age        ;
    //  ltable_proc[old_proc][i].dir         = ltable[i].dir        ;
    //  if (procs[new_proc]) {
    //    ltable[i].confid      = ltable_proc[old_proc][i].confid     ;
    //    ltable[i].CurrentIter = ltable_proc[old_proc][i].CurrentIter;
    //    ltable[i].NbIter      = ltable_proc[old_proc][i].NbIter     ;
    //    ltable[i].TAG         = ltable_proc[old_proc][i].TAG        ;
    //    ltable[i].age         = ltable_proc[old_proc][i].age        ;
    //    ltable[i].dir         = ltable_proc[old_proc][i].dir        ;
    //  }
    //}
    //#endif

    GHIST_proc[old_proc] = GHIST;
    if (procs[new_proc]) {
      GHIST = GHIST_proc[new_proc];
    }
    ptghist_proc[old_proc] = ptghist;
    if (procs[new_proc]) {
      ptghist = ptghist_proc[new_proc];
    }
  ...
  void reinit ()
  {
    ...
    for (int j = 0; j < 100; j++) {
      gtable_proc[j][1] = new gentry[NBBANK[0] * (1 << LOGG)];
      gtable_proc[j][BORN] = new gentry[NBBANK[1] * (1 << LOGG)];
      for (int i = BORN + 1; i <= NHIST; i++)
        gtable_proc[j][i] = gtable[BORN];
      for (int i = 2; i <= BORN - 1; i++)
        gtable_proc[j][i] = gtable[1];
    }


    btable = new bentry[1 << LOGB];
    for (int j = 0; j < 100; j++) {
      btable_proc[j] = new bentry[1 << LOGB];
    }
