...
bt9::BT9Reader bt9_reader_2("/u/p/r/pratyush/private/CS752_Project/traces/SHORT_SERVER-65.bt9.trace.gz");
...
branch func(int iter, int id, branch it, int procs){
	...
	for ( ; it != bt9_reader[id]->end(); ++it) {
        ...
        if((numIter[id] != 0) && (numIter[id] % 9950 == 0)){
...
//                printf("Context Switching now! P0 mispredict rate before switching is 					\t : %10.4f \n",   1000.0*(double)(numMispred)/(double)(numIter));
            fflush(stdout);
		        brpred->store_restore_ghist(((++id)%procs),id);
			break;
		}
        try {
...
//    PREDICTOR  *brpred = new PREDICTOR();  // this instantiates the predictor code
  ...
  ///////////////////////////////////////////////
    ...
    printf("\nProcess | Branch Number | PC | BI | BIndex | GTable | GBank | GIndex | Correct/Wrong Pred | Update(U) BTable | U BIndex | New BTable Pred | U GTable | U GTable Bank | U GTable Index | New Counter | New Useful |\n");
...
//    	printf("Able to Open tracelist");
    ...
    int iterations=min / 10000;
    for(int k=0; k< iterations; k++){
    	for(int p=0;p < procs; p++){
    		resume[p]=func(k, p, resume[p], procs);
    	}
    }
  ///////////////////////////////////////////////
...
//      printf("  NUM_MISPREDICTIONS          \t : %10llu",   numMispred[i]);
      printf(" P%d, %s ,  MISPRED_PER_1K_COND_BR         \t , %10.4f", i, trace_path[i].c_str(),  1000.0*(double)(numMispred[i])/(double)(cond_branch_instruction_counter[i]));
