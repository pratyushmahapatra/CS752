...
branch func(int iter, int id, branch it){
	...
	for ( ; it != bt9_reader[id]->end(); ++it) {
        ...
        if((numIter[id] != 0) && (numIter[id] % switch_interval == 0)){
...
int main(int argc, char* argv[]){
  ...
  if (argc != 4) {
    printf("usage: %s <no. of processes> <tracelist> <ramdom seed>\n", argv[0]);
    exit(-1);
  }
  srand(atoi(argv[3]));
  ///////////////////////////////////////////////
...
//      printf("  NUM_MISPREDICTIONS          \t : %10llu",   numMispred[i]);
      printf(" Random Seed Used: %s,  P%d, %s ,  MISPRED_PER_1K_COND_BR         \t , %10.4f", argv[3], i, trace_path[i].c_str(),  1000.0*(double)(numMispred[i])/(double)(cond_branch_instruction_counter[i]));
